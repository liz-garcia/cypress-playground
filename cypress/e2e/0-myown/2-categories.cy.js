...
describe("Categories", () => {
  ...
  it("Selecting a new category should deselect the old one", () => {
    ...
    cy.get('[data-testid="categories-list"]').should(
      "not.have.class",
      "selected"
    );
    // 2. Click a category
    cy.get('[data-testid="categories-list"]').first().click();

    // 3. Check that that category is selected
    cy.get(
      '[data-testid="categories-list"] [data-testid="category-item"]'
    ).should("have.class", "categories--item-selected");

    // 4. Click a different category
    cy.get('[data-testid="categories-list"] [data-testid="category-item"]')
      .eq(1)
      .click();
